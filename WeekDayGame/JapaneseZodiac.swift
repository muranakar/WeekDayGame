...
struct ZodiacQuiz {
    ...
    var text: String {
        ...
        switch level {
        case .hard: totalText = "今年の\(firstText)の\(secondText)の\(thirdText) の干支は？"
        case .normal: totalText = "今年の\(firstText)の\(secondText) の干支は？"
        case .easy: totalText = "今年の\(firstText) の干支は？"
        }
...
struct ZodiacGame {
    ...
    private let quizLevel: ZodiacQuizLevel
